...
IntervalTimer Pulse;                        // star timer for length of pulse
    ...
    int pulselength;                       // for length of pulse
    int count;                             // time length of pulse 
          void starter(){    
             count=0;   
                if (count<pulselength) {           // call wave1 every 12.5uS untill desired duratoin is reached 
                    wave1.begin(sqwaveGen,12.5);   //call wave generator to occur at 12.5 uS=40kHz
                    count++                        //count wave transitions 
                }else{ 
                  wave1.end()
                }
              }
